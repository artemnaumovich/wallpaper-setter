...
class WallpaperParser(GeneralParser):
    ...
    def precalc_number_of_pictures(self):
        ...
        ) * self.number_of_pictures_on_page + number_of_pictures_on_last_page
    ...
    def get_picture_urls_on_page(self, url):
        ...
        picture_hrefs = [
            item.find("a", class_="wallpapers__link").get("href")
    ...
    def build_page_url_by_number(self, number):
        url = self.PAGE_URL.format(
            category=self.category, resolution=self.resolution, page_number=number
        )
        return url
    ...
    def get_filters(cls):
        ...
        resolutions_links = [
            element.find("a", class_="filter__link") for element in resolutions_list
        ]
        ...
        filters = {"categories": categories, "resolutions": resolutions}

        return filters

    def get_random_picture_url(self):
        picture_number = random.randint(1, self.number_of_pictures)
        page_number = (picture_number + self.number_of_pictures_on_page - 1) // (
            self.number_of_pictures_on_page
        )
        picture_number_on_page = (
            picture_number - (page_number - 1) * self.number_of_pictures_on_page
        )

        page_url = self.build_page_url_by_number(page_number)
        picture_page_url = self.get_picture_page_url_by_number(
            page_url, picture_number_on_page
        )
